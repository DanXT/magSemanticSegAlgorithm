function [ realization] = nonSimpat_Geology1( out,par)
...
if par.hardData
    load('hd.mat');
    hardData((par.szRealization-par.Dim)/2+1:(par.szRealization-par.Dim)/2+par.Dim,(par.szRealization-par.Dim)/2+1:(par.szRealization-par.Dim)/2+par.Dim,:) = hd2(:,:,:,par.i);    
end
...
%             realization = im2bw(realization, level);
            ...
            realization = realization(floor(difd)+1:end-ceil(difd),floor(difd)+1:end-ceil(difd),:);
        end
        m1 = 1;
    end
    ...
    if par.bLoadVariables
        ...
        [X, Locdb] = classifyPatterns_non(out,par);
...
limits  = (par.szRealization  -par.Dim )/2;
limitsz = (par.szRealizationz -par.Dimz)/2;
realization = realization(limits+1:limits+par.Dim , limits+1:limits+par.Dim , :);
% hardData    = hardData   (limits+1:limits+par.Dim , limits+1:limits+par.Dim , limitsz+1:limitsz+par.Dimz);
