...
%                --hp_weight_smoothness_rate 0.0 --hp_weight_decay_rate 0.0001
%
...
nImg = [32 32];
[Y0,C,Ytest,Ctest] = getCFAR10(50000);
%  C = C'; Ctest = Ctest';
...
blocks{end+1} = NN({singleLayer(conv(nImg,[3 3 3 nc]),'activation', act,'Bin',B)});
...
if doTrain || not(exist(resFile,'file'))
    ...
    save(sprintf('%s-run-%d.mat',mfilename,k),'thOpt','WOpt','His1','His2')
    end
else
    load(resFile)
end
